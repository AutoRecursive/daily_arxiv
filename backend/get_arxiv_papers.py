...
def process_raw():
    ...
    return list_ids, list_title, list_authors, list_subjects, list_subject_split

def get_collections(key_words, Key_words, paper, collections='default', save_collections=True):
    '''key_word selection'''
    selected_papers = pd.DataFrame()
    for key_word in key_words:
        selected_paper = paper[paper['title'].str.contains(key_word, case=False)]
        selected_papers = pd.concat([selected_papers, selected_paper], axis=0)
    for Key_word in Key_words:
        selected_paper = paper[paper['title'].str.contains(Key_word, case=True)]
        selected_papers = pd.concat([selected_papers, selected_paper], axis=0)
    if save_collections:
        path = 'data/arxiv_{}/'.format(collections)
        pathlib.Path(path).mkdir(parents=True, exist_ok=True)
        selected_papers.to_json(path+time.strftime("%Y-%m-%d")+'_'+str(len(selected_papers))+'.json')
...
def get_sbj(list_subject_split, items):
    ...
    pathlib.Path(path).mkdir(parents=True, exist_ok=True)
    subject_file.to_json(path+time.strftime("%Y-%m-%d")+'_'+str(len(items))+'.json')
    #subject_file.to_html('subject_file.html')  

def get_arxiv_papers(save_papers=True, save_sbj=True, save_collections=True):
    list_ids, list_title, list_authors, list_subjects, list_subject_split = process_raw()
    items = []
    ...
    if save_papers:
        ...
        pathlib.Path(path).mkdir(parents=True, exist_ok=True)
        paper.to_json(path+time.strftime("%Y-%m-%d")+'_'+str(len(items))+'.json')
    if save_sbj:
    ...
    get_collections(key_words, Key_words, paper, 'Det', save_collections=save_collections)
    get_collections(key_words2, Key_words2, paper, 'MOT', save_collections=save_collections)
